 ...
 * Generates daily activity reports for all users with min. 10 logs
 * Scheduled to run at 20:00 every day
 */
...
export async function generateDailyReport(date: string): Promise<string> {
  ...
  // Get users with minimum 10 logs
  const users = dailyDataStore.getUsersWithMinLogs(10);

  if (users.length === 0) {
    console.log('[ReportGenerator] No users with minimum 10 logs found');
    throw new Error('No users with sufficient activity for report generation');
  }

  // Calculate final KPIs for all users
  users.forEach(userData => {
    dailyDataStore.calculateKPIs(userData.userId);
  });
